...
class SocketIOServer:
    ...
    def __init__(self, config: Dict[str, Any], zmq_manager: ZMQManager):
        ...
        logger.info("SocketIOServer initialized")
    ...
    def register_handlers(self):
        ...
        async def connect(sid, environ):
            ...
            if not self.conn_mgr.can_connect():
                logger.warning(f"Connection rejected: {sid} (another client connected)")
        ...
        async def update(sid, data):
            """UPDATE 指令"""
            logger.info(f"Received UPDATE command from {sid}")
        ...
        async def start(sid, data=None):
            """START 指令"""
            logger.info(f"Received START command from {sid}")
            response = await self._handle_command(sid, {
                'command': 'start'
            })
            return response
        ...
        async def process(sid, data):
            """PROCESS 指令"""
            logger.info(f"Received PROCESS command from {sid}")
            response = await self._handle_command(sid, {
                'command': 'process',
                **data
            })
    ...
    async def _listen_callbacks(self, sid: str):
        ...
        Args:
            sid: 客户端会话ID
        """
        logger.info(f"Started callback listener for {sid}")
        try:
            while True:
                # 检查回调（非阻塞，100ms超时）
                ...
                if callback:
                    logger.info(f"Received callback: {callback}")
                    # 发送回调给客户端
            ...
            logger.info(f"Callback listener cancelled for {sid}")
            raise
        except Exception as e:
